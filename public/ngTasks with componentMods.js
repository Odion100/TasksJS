...
var tasks = (function(window){        
    ...
    console.log(ngService('$templateRequest'))
    function tasks(){        
        var tasks = window.tasks || (window.tasks = {}), app = {}, modules = [], components = [],  backendServices = {}, configurations = {}, initAsync = [], initSync = [];
        ...
        function componentFactory(componentName, modConstructor){
            var component = {};
            component.useService = useService;
            component.useModule = useModule;
            ...
            function useService(serviceName){
                return passService(serviceName, componentName);                      
            } 

            function useModule(modToUse, args){                  
                return getMod(modToUse, args, componentName);                        
            }

            function template(temp){
                component._options.template = temp;
            }
            function templateUrl(url){
                component._options.templateUrl = url;
            }
            modConstructor.apply(component)
            return component

        }
        
        function componentInitializer(componentName, modConstructor){           
            ...
            $scope.$apply(function(){   
                var _component = componentFactory(componentName, modConstructor)            
                $scope[componentName] = _component
                
                if(_component._options.templateUrl){
                    $templateRequest(_component._options.templateUrl)
                ...
                }else if(_component._options.template){
                    $compile($(e).html(_component._options.template).contents())($scope);
                }
        ...
        function addComponent(componentName, modConstructor){                        
            components.push({name:componentName, modConstructor:modConstructor})

           var c = componentFactory(componentName, modConstructor)
        ...
        function loadMod(modToUse, args, modName){
        ...
        function passService(serviceName, modName){
            ...
            console.log(backendServices[serviceName])
            return backendServices[serviceName].service; 
        }
        function modFactory(modName){
            var thisMod = {}, events = {};
        ...
        function modInit(modName, modConstructor){
            ...
            thisMod = new modFactory(modName);              
        ...
        function initModules() {
            ...
            for (var i = 0; i < components.length; i++) {
                componentInitializer(components[i].name, components[i].modConstructor)
            }
        ...
        function createServiceAPI(apiMap, serviceName){
            ...
            for (var i = 0; i < maps.length; i++) {
                ...
                service[maps[i].modName] = new serverModRequestHandler(maps[i], apiMap.host, serviceName) 
        ...
        function serverModRequestHandler(map, host, serviceName){
            ...
            function mapErrHandler(new_api, req, callBack, handler){                    
                ...
                if(attempts >= 3){                
                    ...
                    for (var i = 0; i < new_maps.length; i++) {
                        ...
                        backendServices[serviceName].service[new_maps[i].modName]._updatePath(new_route, new_api.host, new_maps[i].nsp);
                    }
           ...
           var eventHandlers = {};
            ...
            function reconnectService(){                
                ...
                _client.request({
                    method:'GET',
                    url:backendServices[serviceName].uri
    ...
    var _tasks = {app:tasks}, _client = client()    
