...
var storage = multer.diskStorage({
    ...
    filename: function (req, file, cb) {
        cb(null, randomStr(10)+ Date.now() + '.' + mime.getExtension(file.mimetype));        
    }
});

var tasks = {}, maps = [], validateConn, _connPath, _host, _port, _socket_port = parseInt(Math.random()*parseInt(Math.random()*10000))+1023;
...
function init(connPath, port, host, validationFn){
    ...
    console.log('port-----------------')
    console.log(port)
    console.log('--------- '+connPath+" service listening on "+_host+':'+_port);
    ...
    if(validMap(req, res)){
        multiFileHandler(req, res, function(err){                                     
            if(err){                                
                ...
                tasks[app][mod][serverMod][fn](data || {}, function(err, results){
                    if(err){
                        res.status(err.status || 500).json(errorResponseBuilder(err));
                        console.log(err)     
                    }else{
...
function validMap(req, res){
    ...
    function sendMapsErr(){
        res.status(400).json({maps:maps, host:(_host+ ':' + _port), invalidMap:true});  
    }
